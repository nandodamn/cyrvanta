...
export function SecurityTopologyPanel() {
  ...
  const selectedNode =
    nodes.find((n) => n.id === selectedNodeId) ?? (nodes.length > 0 ? nodes[0] : null);
  ...
  const assetNodes = nodes.filter(
    (n) => n.category === "MONITORED_ASSET" || n.id.startsWith("asset-") || n.id.startsWith("lab-"),
  );
  ...
  const filteredNodes = nodes.filter((n) => {
    const matchesSearch =
      ...
      (n.services &&
        n.services.some((s) => s.name.toLowerCase().includes(searchFilter.toLowerCase())));

    const matchesCategory =
      categoryFilter === "ALL" ||
      (categoryFilter === "MONITORED_ASSET" &&
        (n.category === "MONITORED_ASSET" ||
          n.id.startsWith("asset-") ||
          n.id.startsWith("lab-"))) ||
      (categoryFilter === "SECURITY_FEED" &&
        (n.category === "SECURITY_FEED" ||
          n.id.startsWith("integ-") ||
          n.id.startsWith("siem-"))) ||
      (categoryFilter === "CYRVANTA_CORE" &&
        (n.category === "CYRVANTA_CORE" ||
          ["gw-01", "db-01", "telemetry-01", "broker-01", "soar-01", "ai-01"].includes(n.id)));
  ...
  const fitZoom = Math.min(1, VIEWPORT_HEIGHT / canvasHeight, VIEWPORT_WIDTH / VIEWPORT_WIDTH);
      ...
      <div className="topology-panel-header">
        ...
        {nodes.length > 0 && (
          <div className="topology-view-toggle">
            ...
            >
              📋 {i18n.language.startsWith("es") ? "Tabla Consolidada" : "Node Table"} (
              {nodes.length})
            </button>
          </div>
        )}
        ...
        <>
          ...
          <div className="topology-metrics-strip">
            <div className="topology-metric-pill">
              ...
              {i18n.language.startsWith("es") ? "En Línea" : "Online"}:
            ...
            <div className="topology-metric-pill">
              ...
              <strong style={{ color: totalAlerts > 0 ? "var(--warning)" : "var(--text-soft)" }}>
                {totalAlerts}
              </strong>
            </div>
            <div className="topology-metric-pill">
              <span>🏢</span>{" "}
              {i18n.language.startsWith("es") ? "Activos Protegidos" : "Protected Assets"}:
              <strong>{assetNodes.length}</strong>
            </div>
            <div className="topology-metric-pill">
              <span>📡</span>{" "}
              {i18n.language.startsWith("es") ? "Fuentes de Detección" : "Security Feeds"}:
            ...
            <div className="topology-graph-container">
              <div className="topology-zoom-controls">
                ...
                <button
                  ...
                  disabled={isFitted}
                  title={i18n.language.startsWith("es") ? "Ver todo el mapa" : "Fit the whole map"}
                >
              ...
              >
                ...
                <g
                  transform={`translate(${currentView.x} ${currentView.y}) scale(${currentView.zoom})`}
                >
                  {/* 3 Operational Zones */}
                  <g opacity="0.7">
                    ...
                    >
                      📡{" "}
                      {i18n.language.startsWith("es")
                        ? "1. FUENTES DE SEGURIDAD & SENSORES"
                        : "1. SECURITY FEEDS & SENSORS"}
                    </text>
                    <text x="25" y="50" fill="var(--muted)" fontSize="9">
                      {i18n.language.startsWith("es")
                        ? "Ingesta de telemetría (SIEMs, EDRs, Firewalls)"
                        : "Telemetry ingestion (SIEMs, EDRs, Firewalls)"}
                    </text>
                    ...
                    >
                      🛡️{" "}
                      {i18n.language.startsWith("es")
                        ? "2. PLATAFORMA CYRVANTA (NÚCLEO SOC)"
                        : "2. CYRVANTA SECOPS PLATFORM"}
                    </text>
                    <text x="330" y="50" fill="var(--muted)" fontSize="9">
                      {i18n.language.startsWith("es")
                        ? "Correlación, RLS, Almacenamiento e IA"
                        : "Correlation, RLS, Storage and AI Engine"}
                    </text>
                    ...
                    >
                      🏢{" "}
                      {i18n.language.startsWith("es")
                        ? "3. RED Y ACTIVOS PROTEGIDOS"
                        : "3. PROTECTED TENANT ASSETS"}
                    </text>
                    <text x="695" y="50" fill="var(--muted)" fontSize="9">
                      {i18n.language.startsWith("es")
                        ? "Servidores consolidados, servicios y LAN"
                        : "Consolidated hosts, services and LAN"}
                    </text>
                    {assetNodes.length === 0 && (
                  ...
                  <g strokeWidth="2">
                    ...
                    <path
                      d="M 685 220 L 645 220"
                      stroke={warningCount > 0 ? "url(#edgeWarning)" : "url(#edgeNormal)"}
                      strokeDasharray="4 3"
                    />
                  </g>
                  ...
                  {coreNodes.map((n, i) => (
                    ...
                    >
                      ...
                      <text x="28" y="53" fill="var(--accent)" fontSize="8.5">
                        {n.services && n.services.length > 0
                          ? `⚙️ ${n.services.map((s) => s.name).join(" | ")}`
                          : "Core Service"}
                      </text>
                    </g>
                  ))}
                  ...
                  {assetNodes.map((n: TopologyNode, i: number) => {
                    const isWarn =
                      n.status === "WARNING" ||
                      (n.active_alerts_count && n.active_alerts_count > 0);
                    const ipsText =
                      n.ip_addresses && n.ip_addresses.length > 1
                        ? n.ip_addresses.join(" | ")
                        : n.ip_address;
                    const servicesText =
                      n.services && n.services.length > 0
                        ? n.services.map((s) => s.name).join(", ")
                        : "Host Agent";
                    ...
                    return (
                      ...
                      >
                        ...
                        <text x="28" y="60" fill="var(--text-soft)" fontSize="9">
                          ⚙️ {i18n.language.startsWith("es") ? "Servicios" : "Services"}:{" "}
                          {servicesText.length > 34
                            ? servicesText.slice(0, 32) + "..."
                            : servicesText}
                        </text>
              ...
              <div className="topology-legend">
                <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
                  <span>
                    <strong style={{ color: "var(--warning)" }}>
                      🟠 {i18n.language.startsWith("es") ? "Fuentes / Sensores" : "Security Feeds"}
                    </strong>{" "}
                    (Wazuh, EDR, FW)
                  </span>
                  <span>
                    <strong style={{ color: "var(--accent)" }}>
                      🔵{" "}
                      {i18n.language.startsWith("es") ? "Plataforma Cyrvanta" : "Cyrvanta Platform"}
                    </strong>{" "}
                    (SOC Core)
                  </span>
                  <span>
                ...
                <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                  ...
                  <span>
                    ...
                    {i18n.language.startsWith("es") ? "Con Alertas" : "With Alerts"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="topology-table-filter-bar">
                <input
                  ...
                  placeholder={
                    i18n.language.startsWith("es")
                      ? "🔍 Filtrar por host, IP, servicio, tipo..."
                      : "🔍 Filter host, IP, service, type..."
                  }
                  value={searchFilter}
                        ...
                        >
                          <td>
                            <div className="topology-status-cell">
                              <span
                                className={`topology-status-dot ${node.status === "ONLINE" ? "online" : "warning"}`}
                              />
                              <span
                                className={`status ${node.status === "ONLINE" ? "success" : "warning"}`}
                          ...
                          <td>
                            <span
                              className={`topology-category-pill ${getCategoryClass(node.category)}`}
                            >
                          ...
                          <td>
                            ...
                            >
                              {i18n.language.startsWith("es")
                                ? node.role_description_es
                                : node.role_description_en}
                            </div>
                          </td>
                          <td>
                            <span
                            ...
                            <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
                              {node.ip_addresses && node.ip_addresses.length > 0 ? (
                                node.ip_addresses.map((ip) => (
                                  <code key={ip} className="topology-ip-code">
                                    {ip}
                                  </code>
                                ))
                              ) : (
              ...
              >
                <div>
                  ...
                  >
                    ...
                    <span
                      className={`status ${selectedNode.status === "ONLINE" ? "success" : "warning"}`}
                    >
                      {selectedNode.status}
                    </span>
                    ...
                    <span
                      className={`topology-category-pill ${getCategoryClass(selectedNode.category)}`}
                    >
                ...
                <div
                  style={{ display: "flex", gap: "18px", fontSize: "0.85rem", flexWrap: "wrap" }}
                >
                  <div>
                    <span className="muted" style={{ display: "block" }}>
                      {t("topologyIpAddresses")}
                    </span>
                    <div style={{ display: "flex", gap: "4px", marginTop: "2px" }}>
                      {selectedNode.ip_addresses && selectedNode.ip_addresses.length > 0 ? (
                        selectedNode.ip_addresses.map((ip) => (
                          <strong key={ip} className="topology-ip-code">
                            {ip}
                          </strong>
                        ))
                      ) : (
                  ...
                  {selectedNode.monitored_by && selectedNode.monitored_by.length > 0 && (
                    <div>
                      <span className="muted" style={{ display: "block" }}>
                        {t("topologyMonitoredBy")}
                      </span>
                      <strong style={{ color: "var(--cyan)" }}>
                        {selectedNode.monitored_by.join(", ")}
                      </strong>
                    </div>
                  )}
                </div>
                      ...
                      >
                        <div>
                          ...
                          <div style={{ fontSize: "0.72rem", color: "var(--muted)" }}>
                            {svc.protocol} {svc.port ? `• Port ${svc.port}` : ""}{" "}
                            {svc.ip_address ? `• ${svc.ip_address}` : ""}
                          </div>
                        </div>
                        <span
                          className={`status ${svc.status === "ONLINE" ? "success" : "warning"}`}
              ...
              {selectedNode.active_alerts && selectedNode.active_alerts.length > 0 && (
                ...
                >
                  <strong style={{ fontSize: "0.82rem", color: "var(--warning)" }}>
                    ⚠️{" "}
                    {i18n.language.startsWith("es")
                      ? "Alertas de Seguridad Asociadas"
                      : "Associated Security Alerts"}{" "}
