...
- Si una instalación conserva schemas heredados, el catálogo crea una única versión sucesora
  DRAFT, inmutable y auditada; el operador debe validarla y publicarla antes de reconfigurar
  bindings, sin modificar la versión anterior.
...
- Los tres playbooks de egreso autorizados comparten un snapshot real y tenant-scoped con sólo
  ID, código, título, estado, severidad, clasificación, riesgo y resumen de análisis; excluye
  telemetría, evidencia raw, metadatos adicionales, secretos y parámetros libres.
- Un fallo de confirmación SMTP/HTTPS se persiste como `UNKNOWN`, nunca como fracaso definitivo;
  el motor detiene el grafo y conserva esa semántica también al recuperarse de un crash, sin
  reintentar automáticamente efectos externos no seguros.
...
- Aprobar o rechazar exige una justificación escrita por el analista, validada también en backend;
  no se generan motivos prefabricados y la etiqueta de cuatro ojos sólo aparece con quórum doble.
...
El operador autorizó explícitamente el 2026-08-13 que estos tres playbooks de egreso puedan habilitarse: `notify-critical-incident`, `create-security-ticket` e `incident-report-email`. La autorización no activa bindings, conexiones, aprobaciones ni switches LIVE, y no provoca ejecuciones automáticas.
